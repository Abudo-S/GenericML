...
if __name__ == "__main__":
    ...
    svm_params_combs = [{'learning_rate': 0.001, 'lambda_param': 0.01, 'n_iterations': 50},
                        {'learning_rate': 0.001, 'lambda_param': 0.01, 'n_iterations': 30},
                        {'learning_rate': 0.001, 'lambda_param': 0.01, 'n_iterations': 20}]

    kcv = KCrossValidation(model_class=SVM)

    # Run k-fold cross-validation
    cv_scores = kcv.evaluate_params_combinations(
        X=X_train,
        y=y_train,
        model_params_list=svm_params_combs
    )
